...
export default class MAIN extends CONTAINER {
	...
	*/
	loadAjaxCall(data) {
		if (data.result === 1) {
			...
			this.loader.log(0, 'Failed to retrieve ' + this.className + '(' + id + ') from server\n' + data.message);
			this.loader.showConsole();
		}
	}
	/**
	...
	*/
	login(email, password) {
		...
		this.email = new INPUT(this.prompt.form.fieldset.formElementGroup.body.pane, new MODEL(new ATTRIBUTES({
		...
		this.password = new INPUT(this.prompt.form.fieldset.formElementGroup.body.pane, new MODEL(new ATTRIBUTES({
	...
	*/
	register() {
		...
		this.passwordConfirm = new INPUT(this.prompt.form.fieldset.formElementGroup.body.pane, new MODEL(new ATTRIBUTES({
			'typeId': INPUTTYPES.PASSWORD,
			'type': 'Password',
