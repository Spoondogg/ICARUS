...
class MODAL extends EL { // ALIGN VERTICALLY??
     ...
     */
    constructor(label, text, vertical) {
        ...
        this.header.text = new EL(this.header.main, 'DIV', new MODEL(new ATTRIBUTES('text')), label);

        this.header.btnClose.el.onclick = this.close.bind(this);

        // Body	: Place forms, objects etc inside 'this.modalbody.el'
        this.body = new EL(this.content, 'DIV', new MODEL(new ATTRIBUTES('modal-body')));

        // A well containing various alerts
        this.text = new P(this.body, new MODEL(new ATTRIBUTES('well')), text);
    ...
    */
    addText(text) {
        if (text) {
            this.text.el.innerHTML += '<br>' + text;
        }
    }
}